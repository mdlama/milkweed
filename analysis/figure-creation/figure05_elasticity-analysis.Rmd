...
if (compute) {
  ...
  save(results, file=file.path(mwCache,"figure06.rda"))
} else 
  if (local) {
    load(file.path(mwCache,"figure06.rda"))
  } else {
...
#dist.plot.data = dist.plot.data %>% mutate(pointtype = ifelse(name == 'mean', 'mu', ifelse(name == 'sd', 'sigma', 'p')))
```
...
dist.plot <- dist.plot.data %>% 
  ...
  geom_vline(xintercept = 0, 
             ...
             size = 0.2) +
  scale_color_manual(values = group.colors) +
  geom_point() + #insert aes(shape = pointtype) to make different shapes
  geom_errorbarh(aes(xmin = meanElasticity - ci,
                     xmax = meanElasticity + ci),
                height = 0.3)
...
# Individual panel Point types
#vr.plot.data = vr.plot.data %>% mutate(pointtype = ifelse(newtype == 'Constants', 'alpha', 'beta'))
```
...
### Put panels together
Put 'em all together with gridExtra!
```{r}
#issues with outputting the right dimensions and legend scaling... did this in inkscape instead

grid.arrange(dist.plot, vr.plot, nrow=2) #visualize

g = arrangeGrob(dist.plot, vr.plot, nrow=2) #generates g
```
