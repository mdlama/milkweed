...
Winner:
```{r}
# winner <- pods.fe.haPhe.re.site_ha.year_he
winner <- pods.fe.haPhaIhe.re.site_ha.year_he
summary(winner)
```
...
```{r}
par(mfrow=c(2,2))
plot(fitted(winner), residuals(winner, "deviance"), xlab="Fitted values", ylab="Deviance residuals")
plot(metadata_sc$h_apical, residuals(winner, "deviance"), xlab="Apical height", ylab="Deviance residuals")
plot(metadata_sc$herb_avg, residuals(winner, "deviance"), xlab="Herbivory average", ylab="Deviance residuals")
par(mfrow=c(1,1))
```

DHARMa Diagnostics
```{r}
pods.simulationOutput = DHARMa::simulateResiduals(fittedModel = winner, n=1000)
```
