...
# Need to handle all variables (even those that are zero statistically)
...
newlabelorder <- c("Clonal-a", 
                   ...
                   "Flowering-h_apical", 
                   "Flowering-herb_avg", 
                   "Flowering-h_apical:herb_avg",
                   "Pods-(Intercept)", 
                   "Pods-h_apical.next", 
                   "Pods-herb_avg",
                   "Pods-h_apical.next:herb_avg",
                   "Seedlings-mean", 
                   "Seedlings-sd", 
                   ...
                   "Growth-h_apical", 
                   "Growth-herb_avg",
                   "Growth-h_apical:herb_avg",
                   "Growth-sd", 
                   "Survival-(Intercept)",
                   "Survival-h_apical",
                   "Survival-herb_avg",
                   "Survival-h_apical:herb_avg")
...
fancypantslabels[13] <- TeX('$\\beta_{z_{h}^{\\prime}}$')
fancypantslabels[14] <- TeX('$\\beta_{z_{\\omega}}$')
fancypantslabels[15] <- TeX('$\\beta_{z_{h}^{\\prime}:z_{\\omega}}$')

# Seedlings
fancypantslabels[16] <- TeX('$\\mu$')
fancypantslabels[17] <- TeX('$\\sigma$')
...
# Growth
fancypantslabels[20] <- TeX('$\\alpha$')
fancypantslabels[21] <- TeX('$\\beta_{z_{h}}$')
fancypantslabels[22] <- TeX('$\\beta_{z_{\\omega}}$')
fancypantslabels[23] <- TeX('$\\beta_{z_{h}:z_{\\omega}}$')
fancypantslabels[24] <- TeX('$\\sigma$')

# Survival
fancypantslabels[25] <- TeX('$\\alpha$')
fancypantslabels[26] <- TeX('$\\beta_{z_{h}}$')
fancypantslabels[27] <- TeX('$\\beta_{z_{\\omega}}$')
fancypantslabels[28] <- TeX('$\\beta_{z_{h}:z_{\\omega}}$')
...
ym <- 0.25
al <- 0.1
r_survival <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1-ym, ymax = 4+ym, fill = "black", alpha = al)
r_growth <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 5-ym, ymax = 9+ym, fill = "black", alpha = al)
r_seedlings <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 12-ym, ymax = 13+ym, fill = "black", alpha = al)
r_pods <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 14-ym, ymax = 17+ym, fill = "black", alpha = al)
r_flowering <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 18-ym, ymax = 21+ym, fill = "black", alpha = al)
r_herbivory <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 22-ym, ymax = 24+ym, fill = "black", alpha = al)
r_budlings <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 25-ym, ymax = 26+ym, fill = "black", alpha = al)
r_bdlgs_per_stem <- annotate("rect", xmin = -Inf, xmax = Inf, ymin = 27-ym, ymax = 28+ym, fill = "black", alpha = al)

# Functional form labels
xc <- -0.225
sz <- 2.5
t_bdlgs_per_stem <- annotate("text", x = xc, y = 27.5, label = "Buds per stem", size = sz)
t_budlings <- annotate("text", x = xc, y = 25.5, label = "Budling recruits", size = sz)
t_herbivory <- annotate("text", x = xc, y = 23, label = "Herbivory", size = sz)
t_flowering <- annotate("text", x = xc, y = 19.5, label = "Flowering", size = sz)
t_pods <- annotate("text", x = xc, y = 15.5, label = "Pods", size = sz)
t_seedlings <- annotate("text", x = xc, y = 12.5, label = "Seedling recruits", size = sz)
t_seed_surv <- annotate("text", x = xc, y = 11, label = "Seed survival", size = sz)
t_seeds_per_pod <- annotate("text", x = xc, y = 10, label = "Seeds per pod", size = sz)
t_growth <- annotate("text", x = xc, y = 7, label = "Growth", size = sz)
t_survival <- annotate("text", x = xc, y = 2.5, label = "Ramet survival", size = sz)
...
```{r}
maxval <- max(abs(c(as.vector(E1), as.vector(E2))))
...
pmunch_elastiplot <- ggplot(plotdata, aes(x = h_apical_t0, 
  ...
  facet_grid(factor(pname, levels=c("p[omega]", "mu")) ~ ., labeller=label_parsed) +
