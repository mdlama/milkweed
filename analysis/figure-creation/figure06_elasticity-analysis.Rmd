...
plotdata <- results %>%
  ...
  mutate(elasticity = abs(pars)*sensitivity/lambda) %>% 
  group_by(label) %>% 
...
elastiplot <- elastiplot + 
  ...
  theme(legend.background = element_rect(fill="lightgrey",
        ...
        legend.position = c(0.89, 0.145),
        plot.title = element_text(hjust = 0,
                                  margin = margin(b = 15, unit = "pt")))

elastiplot
ggsave("Elasticities_new.png", device="png", elastiplot)
```
...
```{r, fig.width=8, fig.height=4, eval=FALSE, echo=FALSE}
