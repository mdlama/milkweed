...
ps <- plotdata2 %>% ggplot(aes(x = herb_avg,
                             y = growth_rate)) +
                  geom_line() + 
...
ps <- ps + geom_point(aes(x = herb_avg, 
                        y = growth_rate, 
                        color = site), 
                    size = 0.1,
                    alpha = 0.5,
...
# Bertha growth rate vs point-mass herbivory scores
py <- plotdata2 %>% ggplot(aes(x = herb_avg,
                             y = growth_rate)) +
                  geom_line() + 
                  geom_hline(aes(yintercept = 1.0), 
                             linetype = 3)
...
# Site-specific bootstrapped data points with 95% confidence ellipse
py <- py + geom_point(aes(x = herb_avg, 
                        y = growth_rate, 
                        color = year), 
                    size = 0.1,
                    alpha = 0.5,
...
```{r}
# ggsave("Figure5pt2_PopGrowthVsHerb.png", height=4, width=6, device = "png", p)
```

```{r, fig.width=12, fig.height=4}
pall <- gridExtra::grid.arrange(ps, py, ncol=2)
ggsave("Figure6_PopGrowthVsHerb.png", height=4, width=12, device = "png", pall)
```
