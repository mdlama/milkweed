...
if (FALSE) { # if (any(abs(qgadj) > 0)) {
  pg <- pg + annotate("point", 
                      x = growth.fit$pars$unscaled["Bertha", "herb_avg"] + qgadj[1], # qgadj[1]-qgadj[2], 
                      y = 0.55, 
                      shape=25) +
    annotate("point", 
             x = growth.fit$pars$unscaled["Bertha", "herb_avg"] + qgadj[3], # qgadj[3]-qgadj[2], 
             y = 0.55, 
             shape=25, 
             fill="black")
}
