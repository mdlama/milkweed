...
# Heatmap
p5 <- ggplot(mydata, aes(x = h_apical, 
                         y = herb_avg, 
                         z = prob.surv)) +
      geom_raster(aes(fill = prob.surv)) +
      geom_contour(colour = "white", alpha = 0.8) + 
...
p5 <- p5 + geom_hline(aes(yintercept = yintercept), 
                      data = herb_ex, 
                      linetype = c(1, 2, 5), 
                      col = RColorBrewer::brewer.pal(5, "YlOrRd")[2:4],
                      size=1)
...
p6 <- ggplot(mycurves, aes(x = h_apical, 
                           y = prob.surv, 
                           col = Herbivory, 
                           linetype = Herbivory)) +
      geom_line(size = 1.0) + 
...
```{r}
# Prepare data for heatmap
mydata <- tbl_df(data.frame(h_apical.next = as.vector(M$x),
                            herb_avg = as.vector(M$y),
...
# Heatmap
p7 <- ggplot(mydata, aes(x = h_apical.next, 
                         y = herb_avg, 
                         z = N_pods)) +
      geom_raster(aes(fill = N_pods)) +
      geom_contour(colour = "white", alpha = 0.8) + 
      scale_fill_gradientn("Height\n(Late Season)", 
                           colours=c("#00000000","#BBBBBBBB"),
                           limits=c(0, max(mydata$N_pods)))

# For some reason, coord_cartesian didn't work (left gap between tick marks and image)
p7 <- p7 + theme(axis.line = element_blank()) + 
     scale_x_continuous(limits = c(0, max(M$x)), 
                        expand = c(0, 0)) + 
     scale_y_continuous(limits = c(0, 6), 
                        expand = c(0, 0)) +
...
p7 <- p7 + geom_hline(aes(yintercept = yintercept), 
                      data = herb_ex, 
                      linetype = c(1, 2, 5), 
                      col = RColorBrewer::brewer.pal(5, "YlOrRd")[2:4],
                      size=1)
...
p8 <- ggplot(mycurves, aes(x = h_apical.next, 
                           y = N_pods, 
                           col = Herbivory, 
                           linetype = Herbivory)) +
      geom_line(size = 1.0) + 
...
ggsave("Figure2_Appendix_1.png", width=7, height=5, device = "png", p)
```
```{r}
g5 <- ggplotGrob(p5)
g6 <- ggplotGrob(p6)
g6 <- gtable::gtable_add_cols(g6, unit(0, "mm"))
g6 <- gtable::gtable_add_cols(g6, unit(0, "mm"))
g <- rbind(g5, g6, size="first")
g$widths <- grid::unit.pmax(g5$widths, g6$widths)
 
g7 <- ggplotGrob(p7)
g8 <- ggplotGrob(p8)
g8 <- gtable::gtable_add_cols(g8, unit(0, "mm"))
g8 <- gtable::gtable_add_cols(g8, unit(0, "mm"))
gg <- rbind(g7, g8, size="first")
gg$widths <- grid::unit.pmax(g7$widths, g8$widths)

p <- gridExtra::grid.arrange(g, gg, ncol=2)
# p <- gridExtra::grid.arrange(p2, p4, ncol=2)
ggsave("Figure2_Appendix_2.png", width=7, height=5, device = "png", p)
```
