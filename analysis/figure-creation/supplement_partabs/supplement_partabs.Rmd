...
metadata <- dem.data %>% filter(!is.na(h_apical), !is.na(munched))
N_munched <- sum(metadata$munched == 1)
N <- nrow(metadata)
pmunch <- unname(binom.agresti.coull(N_munched, N))
CI <- unname(summary(fitdistrplus::bootdist(munched.fit$fit))$CI)
df <- rbind(df,
  data.frame(Model = rep("Herbivory", 3),
           Parameter = c("$p_{\\omega}$", "$\\mu$", "$\\sigma$"),
           Lower = c(pmunch[[5]], CI[,2]),
           Value = c(pmunch[[4]], unname(munched.fit$fit$estimate)),
           Upper = c(pmunch[[6]], CI[,3]))
)
