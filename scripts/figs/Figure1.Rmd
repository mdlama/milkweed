---
title: "Figures for Sivan's Paper"
date: "June 2, 2016"
output: 
  html_document:
    toc: true
...
mydata <- tbl_df(data.frame(h_apical = rep(as.vector(M$x), 6),
                            log_herb_avg = rep(as.vector(M$y), 6)))
z <- rep(0, nrow(mydata))
for (i in 1:6) {
  z[(1+100*100*(i-1)):(100*100*i)] <- predict(vital.fit, 
                                              newdata=data.frame(h_apical = as.vector(M$x),
                                                                 log_herb_avg = as.vector(M$y)),
                                              type=rownames(vital.fit$pars$scaled)[i])
}
mydata <- mydata %>% mutate(prob.flower = z) 
mydata$site <- rep(rownames(vital.fit$pars$scaled), each=100*100)

mydata %>% ggplot(aes(x = h_apical, y = log_herb_avg, z = prob.flower)) +
...
Visualization doesn't really help us here.

```{r, echo=FALSE}
formula(surv.fit$mdl)
...
mydata %>% ggplot(aes(x = h_apical, y = log_herb_avg, z = h_apical.next)) +
...
```
h_apical.next ~ h_apical*log_herb_avg + (h_apical + log_herb_avg | site/transect) + (h_apical | year)
```

```{r, echo=FALSE}
growth.fit$pars$unscaled
```
...
mydata <- tbl_df(data.frame(h_apical.next = rep(as.vector(M$x), 6),
                            log_herb_avg = rep(as.vector(M$y), 6)))
z <- rep(0, nrow(mydata))
for (i in 1:6) {
  z[(1+100*100*(i-1)):(100*100*i)] <- predict(vital.fit, 
                                              newdata=data.frame(h_apical.next = as.vector(M$x),
                                                                 log_herb_avg = as.vector(M$y)),
                                              type=rownames(vital.fit$pars$scaled)[i])
}
mydata <- mydata %>% mutate(log_N_pods = log(z)) 
mydata$site <- rep(rownames(vital.fit$pars$scaled), each=100*100)

mydata %>% ggplot(aes(x = h_apical.next, y = log_herb_avg, z = log_N_pods)) +
  geom_raster(aes(fill = log_N_pods)) +
  geom_contour(colour = "white") + 
  facet_grid(site ~ .)
```

```
N_pods ~ h_apical.next + log_herb_avg + (h_apical.next + log_herb_avg | site/transect) + (h_apical.next + log_herb_avg | year)
```

```{r, echo=FALSE}
pods.fit$pars$unscaled
```
