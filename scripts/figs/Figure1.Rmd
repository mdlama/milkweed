...
y <- seq(from = -2.5, to = 2, length.out = 100)
M <- mesh(x, y)
```
...
# Add lines
herb_ex <- data.frame(yintercept = c(log(0.1), 
                                     mean(log_herb_avg), 
                                     log(6.1)))
...
log_herb_avg <- ipm$data %>% 
  filter(!is.na(h_apical),
         ...
         surv == 1) %$%
  log_herb_avg
M <- mesh(ipm$vars$h_apical$x, ipm$vars$log_herb_avg$x)
```
...
ggsave("Figure1.png", width=7, height=5, device = "png", p)
```