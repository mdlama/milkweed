...
```{r}
metadata=read.csv("~/Google Drive/Project :: ipmwam/Papers/SivansPaper/Milkweed/DATA SETS/Data Frames/metadata13to15.csv")
...
Now plot.
```{r, echo=FALSE}
histogram(~ log(0.1+herb_avg)|site, data=metadata, type = c("density"), munched.fit = munched.fit, panel = function(x,y) {
  bmyx <- seq(from = log(0.1), to = log(6.1), length.out=101)
  myx <- 0.5*(bmyx[1:100] + bmyx[2:101])
  pn <- packet.number()
  panel.histogram(x, breaks=20)
  panel.lines(myx, munched.fit[[pn+1]]$predict(bmyx), col="red", lwd=2)
})
```

Save results to file.
```{r}
save(munched.fit, file = "munchedFit.RData")
```