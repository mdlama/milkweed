...
# Tried indicator variable for herb_avg - close to just log(c+x), so not using
# metadata <- metadata %>% mutate(surv = ifelse(is.na(surv) & is.na(h_apical.next) & !is.na(h_apical), 0, surv), 
                                #munched = ifelse(herb_avg > 0, 1, 0),
                                #herb_avg = ifelse(munched == 1, log(herb_avg), herb_avg))
# See: http://stats.stackexchange.com/questions/4831/logistic-regression-transforming-variables
```
...
xyplot(jitter(fec.flower, factor=0.2) ~ h_apical|site, data=metadata_sc, group=year, auto.key = TRUE)
```
