...
metadata <- read.csv(mwROOT("data","stemdata.csv"))
...
Full model first.  Use nAGQ = 0 for quick approximations.
```{r}
...
Let's try and simplify `site/transect` random effect first, but we'll keep `log_herb_avg` in as that is our main interest.
```{r}
...
bictab(list(flower.reg.mdl1, flower.reg.mdl2, flower.reg.mdl3),
       modnames = c("mdl1","mdl2","mdl3"))
# AIC(flower.reg.mdl1, flower.reg.mdl2, flower.reg.mdl3)
# BIC(flower.reg.mdl1, flower.reg.mdl2, flower.reg.mdl3)
```

Picking `mdl3` with `(1|site/transect)`, i.e. site/transect on intercept only.  Let's move to `year`.
...
flower.reg.mdl6=glmer(fec.flower ~ h_apical*log_herb_avg + (1|site/transect) + (log_herb_avg|year), data=metadata_sc, nAGQ=0, family=binomial())
flower.reg.mdl7=glmer(fec.flower ~ h_apical*log_herb_avg + (1|site/transect) + (1|year), data=metadata_sc, nAGQ=0, family=binomial())

aictab(list(flower.reg.mdl3, flower.reg.mdl4, flower.reg.mdl5, flower.reg.mdl6, flower.reg.mdl7),
       modnames = c("mdl3","mdl4","mdl5","mdl6","mdl7"))

bictab(list(flower.reg.mdl3, flower.reg.mdl4, flower.reg.mdl5, flower.reg.mdl6, flower.reg.mdl7),
       modnames = c("mdl3","mdl4","mdl5","mdl6","mdl7"))

# AIC wants Model #3, BIC wants Model #4 - Let's got with Model #4
...
flower.reg.mdl3 = glmer(fec.flower ~ log_herb_avg + h_apical:log_herb_avg + (1|site/transect)+(h_apical+log_herb_avg|year), data=metadata_sc, nAGQ=1, family=binomial(), control=glmerCtrl)
flower.reg.mdl4 = glmer(fec.flower ~ h_apical + (1|site/transect)+(h_apical+log_herb_avg|year), data=metadata_sc, nAGQ=1, family=binomial(), control=glmerCtrl)

aictab(list(flower.reg.winner, flower.reg.mdl1, flower.reg.mdl2, flower.reg.mdl3, flower.reg.mdl4),
       modnames = c("re_winner","mdl1","mdl2","mdl3","mdl4"))

bictab(list(flower.reg.winner, flower.reg.mdl1, flower.reg.mdl2, flower.reg.mdl3, flower.reg.mdl4),
       modnames = c("re_winner","mdl1","mdl2","mdl3","mdl4"))
```

Looks like Model #2 is the clear winner via BIC.  Let's check P-values and create class to verify that unscaled version is correct.
```{r}
...
```{r}
par_usc <- as.data.frame(flower.fit$pars$unscaled)
...
# ypts = 1/(1+exp(-1*(pars$unscaled['(Intercept)'] + pars$unscaled['h_apical']*xpts + pars$unscaled['log_herb_avg']*meanha + pars$unscaled['h_apical:log_herb_avg']*xpts*meanha)))
ypts = 1/(1+exp(-1*(par_usc['Bertha','(Intercept)'] + par_usc['Bertha','h_apical']*xpts + par_usc['Bertha','log_herb_avg']*meanha + par_usc['Bertha','h_apical:log_herb_avg']*xpts*meanha)))
...
meanha <- mean(metadata_usc$log_herb_avg)
# ypts = 1/(1+exp(-1*(pars$unscaled['(Intercept)'] + pars$unscaled['h_apical']*xpts + pars$unscaled['log_herb_avg']*meanha + pars$unscaled['h_apical:log_herb_avg']*xpts*meanha)))
ypts = 1/(1+exp(-1*(par_usc['Bertha','(Intercept)'] + par_usc['Bertha','h_apical']*xpts + par_usc['Bertha','log_herb_avg']*meanha + 
lines(xpts, ypts, col="black", lwd=2, lty=2)
legend("bottomright", sort(unique(as.character(metadata_usc$site))), col=1:5, pch=1)
```
