...
```{r}
data_gp <- stemdata %>% group_by(year, transect) %>% 
...
Both models are rather similar - in this case we will go with the exponential model, as it makes more biological sense (it won't go negative!!!)
```{r}
fit <- exp.model 
